...
<script type="text/javascript">
    document.getElementById("searchBtn").onclick = function() {
		...
		<table class="bbsList">
		<c:if test="${id != null && id != 'myCs.mem_id'}">
                ...
                <tbody class="body">
                  ...
                  <c:forEach var="vo" items="${myCs}">	 
                     <input type="hidden" name="mem_id" value="${vo.mem_id}">
                    <tr>
                       <td class="">${vo.cs_num }</td>                       
                        <td class="">${vo.cs_category}</td>
                        <td class="">${vo.cs_process}</td>
                  <%--       ${blist.cs_secret}     --%>                    
                                
                <td>
                   <c:choose>
                      <c:when test="${vo.cs_secret == true && (id != '${vo.mem_id}')}">
                      <span>비밀글입니다.</span>
                      </c:when>
                   <c:otherwise>
                       <a href="/cs/csread?cs_num=${vo.cs_num}">${vo.cs_sub }</a>
                   </c:otherwise> 	
               </c:choose>
             </td> 
                       
                       
                        <td class="">${vo.mem_id}</td>
                        <td class="">${vo.cs_date }</td>                                                                                                                             
                    </tr> 
